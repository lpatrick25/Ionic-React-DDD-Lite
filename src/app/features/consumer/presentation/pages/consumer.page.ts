...
export class ConsumerPage implements OnInit {
    ...
    request$
      ...
      .subscribe({
        next: (consumer: ConsumerEntity) => {
          console.log('Consumer saved successfully:', consumer);
  ...
  async onDeleteConsumer(consumerId: number) {
    console.log('Delete consumer clicked:', consumerId);
  ...
  private async handleDeleteConsumer(consumerId: number) {
    console.log('Processing delete for consumer:', consumerId);
    this.isLoading$.next(true);
  ...
  onSearch(query: string) {
    ...
    this.searchQuerySubject.next(query); // Fixed: Use the actual query
